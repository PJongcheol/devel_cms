	...
	<!-- 자식메뉴 분기처리 -->
		<c:choose>
			<c:when test="${empty item.children }">
				<li>
					<a href="javascript:void(0)" onclick="fn_goMenu('${item.menuUrl}', '${item.menuId }', '${item.menuPid }')" class="menu-link">
				...
				<li class="has-submenu">
					...
					<ul class="submenu">
						<c:forEach var="children" items="${item.children }">
							<a href="javascript:void(0)" onclick="fn_goMenu('${children.menuUrl}', '${children.menuId }', '${children.menuPid }')" class="menu-link">
